...
struct PowResult { long long value; };

static string to_binary(unsigned long long e) {
    if (e == 0) return "0";
    string s;
    while (e) { s.push_back((e & 1) ? '1' : '0'); e >>= 1; }
...
int main() {
    ...
    for (long long cand = 2; cand < n; ++cand) {
        if (mdc_verbose(cand, phi) == 1) { E = cand; break; }
    }
    if (E == 0) { cout << "2.4. Falha ao escolher E valido. Sem solucao valida.\n"; return 0; }
    ...
    if (plain_codes.empty()) { cout << "3.2. Nada para codificar. Fim.\n"; return 0; }
    ...
    for (char c : msg) {
        if (c == ' ') norm.push_back(' ');
        else if (isalpha((unsigned char)c)) norm.push_back(toupper((unsigned char)c));
    }
